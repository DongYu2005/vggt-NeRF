...
class MVSSystem(LightningModule):
    ...
    def init_volume(self):
        ...
        ckpts = torch.load(self.args.ckpt)
        if 'volume' not in ckpts.keys():
            self.MVSNet.train()
            with torch.no_grad():
                volume_feature, _, _ = self.MVSNet(self.imgs, self.proj_mats, self.near_far_source, pad=self.args.pad, lindisp=self.args.use_disp)
        else:
        ...
        if self.args.use_color_volume or self.args.use_density_volume:
            D,H,W = volume_feature.shape[-3:]
            intrinsic, c2w = self.pose_source['intrinsics'][0].clone(), self.pose_source['c2ws'][0]
            intrinsic[:2] /= 4
            vox_pts = get_ptsvolume(H-2*self.args.pad,W-2*self.args.pad,D, self.args.pad,  self.near_far_source, intrinsic, c2w)
            ...
            if self.args.use_density_volume:
    ...
    def validation_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            ...
            # ================= [开始修改这里] =================
            scan_name = os.path.basename(self.args.datadir.strip('/'))
            # 1. 拼接一个新的文件夹路径，包含当前的 global_step
            # 这样每次验证（比如 step 500, 1000...）都会创建不同的文件夹
            step_save_dir = f'runs_fine_tuning/{scan_name}/{self.args.expname}/step_{self.global_step:08d}/'
            os.makedirs(step_save_dir, exist_ok=True)

            # 2. 准备要保存的图片数据
            # 拼合顺序：GT图 | 预测RGB | 误差图 | 预测深度图
            img_vis_save = torch.cat((img, rgbs, img_err_abs*10, depth_r.permute(1,2,0)), dim=1).numpy()
            
            # 3. 保存图片
            # 文件名使用 batch_nb (即当前视角的编号)，这样文件夹里就是干净的 000.png, 001.png...
            save_path = f'{step_save_dir}/{batch_nb:03d}.png'
            
            imageio.imwrite(save_path, (img_vis_save*255).astype('uint8'))
    ...
    def on_validation_epoch_end(self):
        # 1. 从列表里取出本轮所有 step 的结果
        outputs = self.validation_step_outputs
        
        # 2. 加上非空判断，防止 validation 没跑导致报错
        if len(outputs) > 0:
            ...
            if self.args.with_depth:
                ...
                # 注意：这里需要确保 mask_sum 不为 0，防止除以 0
                if mask_sum > 0:
    ...
    def save_ckpt(self, name='latest'):
        ...
        # 2. 路径里加入 scan_name
        # 结果路径: runs_fine_tuning/scan4/你的实验名/ckpts/
        save_dir = f'runs_fine_tuning/{scan_name}/{self.args.expname}/ckpts/'
...
if __name__ == '__main__':
    ...
    scan_name = os.path.basename(args.datadir.strip('/'))
    save_path = os.path.join("runs_fine_tuning", scan_name, args.expname, "ckpts")
    checkpoint_callback = ModelCheckpoint(dirpath=save_path, 
                                          filename='{epoch:02d}',
                                          monitor='val/PSNR_all',
                                          mode='max',
                                          save_top_k=1,
                                          save_last=True)
    ...
    logger = loggers.TensorBoardLogger(
        save_dir=os.path.join("runs_fine_tuning", scan_name),
        name=args.expname
    )
    ...
    trainer = Trainer(
        ...
        callbacks=[checkpoint_callback],  # ✅ 已修正
        logger=logger,
        
        # --- 硬件设置 (新版写法) ---
        accelerator='gpu',                # 代替 gpus
        devices=args.num_gpus,            # 代替 gpus
        strategy='ddp' if args.num_gpus > 1 else 'auto', # 代替 distributed_backend
